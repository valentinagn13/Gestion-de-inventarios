...
class buscarPrecio_Categoria:
    def __init__(self, master, inventario):
        ...
        # Etiqueta de título
        label = tk.Label(self.frame, text="Buscar Productos por Precio y Categoría", font=("Arial", 14))
        label.pack(pady=10)
    ...
    def buscar_productos(self):
        ...
        resultados_precio = self.inventario.buscar_por_rango(precio_min, precio_max)
        resultados_categoria = self.inventario.buscar_por_categoria(categoria)

        # Filtrar resultados para obtener solo los que cumplen ambas condiciones
        resultados = [producto for producto in resultados_precio if producto in resultados_categoria]
        ...
        if resultados:
            ...
            self.label_info.config(text="No se encontraron productos en este rango y categoría.")
